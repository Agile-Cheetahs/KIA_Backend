...
# shopping list table#shoppingg list table
class ShoppingListItem(models.Model):
    ...
    unit = models.CharField(max_length=10, choices=UNITS_CHOICES, default='count') 
    shopping_list = models.ForeignKey('ShoppingList', on_delete=models.CASCADE, related_name='items',null=True )


    crossed = models.BooleanField(default=False)

    def __str__(self):
        return f"{self.name} - {self.quantity} {self.unit} - {'Crossed' if self.crossed else 'Not Crossed'}"
